...
Deployment mode
---------------
When the raw CSVs are not present (e.g. on Streamlit Cloud), the module
automatically falls back to pre-aggregated files in `deployment/data_precomputed/`.
Run `deployment/precompute.py` once locally to generate those files.
"""
...
# Pre-aggregated fallback files (generated by precompute.py, committed to git)
PRECOMPUTED_DIR = Path(__file__).resolve().parent / "data_precomputed"

# Auto-detect which mode we're in
_RAW_AVAILABLE         = all((DATA_DIR / f).exists() for f in CSV_FILES)
_PRECOMPUTED_AVAILABLE = (PRECOMPUTED_DIR / "kpi.json").exists()
...
def load_csv():
    """Only available when raw CSVs are present. Raises RuntimeError otherwise."""
    if not _RAW_AVAILABLE:
        raise RuntimeError(
            "Raw CSV files not found. Use load_kpi() / load_shopping_heatmap() "
            "for precomputed data, or run precompute.py first."
        )
...
def dept_color_map():
    ...
    if _RAW_AVAILABLE:
        ...
        ps = product_stats()
        dept_list = sorted(ps["department"].unique())
...
def aisle_aggregates():
    ...
    In precomputed mode, loads from data_precomputed/aisle_aggregates.csv.
    """
    if not _RAW_AVAILABLE:
        return pd.read_csv(PRECOMPUTED_DIR / "aisle_aggregates.csv")
...
def product_stats():
    ...
    In precomputed mode, loads from data_precomputed/product_stats.csv.
    """
    if not _RAW_AVAILABLE:
        return pd.read_csv(PRECOMPUTED_DIR / "product_stats.csv")
...
def load_aisle_pairs():
    ...
    In precomputed mode, loads from data_precomputed/aisle_pairs.json
    (generated by precompute.py). This avoids reading the 551 MB
    order_products__prior.csv at runtime.
    """
    if not _RAW_AVAILABLE:
        pairs_raw = json.loads((PRECOMPUTED_DIR / "aisle_pairs.json").read_text())
        # JSON keys are "{a},{b}" strings — convert back to (int, int) tuples
        pairs = {
        ...
        count_a = {
            ...
            for k, v in json.loads(
                (PRECOMPUTED_DIR / "aisle_count_a.json").read_text()
            ).items()
        }
        n_orders = json.loads((PRECOMPUTED_DIR / "aisle_n_orders.json").read_text())
        aid2name = {
            int(k): v
            for k, v in json.loads(
                (PRECOMPUTED_DIR / "aid2name.json").read_text()
            ).items()
        }
        return pairs, count_a, n_orders, aid2name
...
def aisle_enrichment():
    ...
    cluster naming.
    In precomputed mode, uses aisle_aggregates.csv which already contains
    the aisle_id column.
    """
    ...
    if _RAW_AVAILABLE:
        ...
        # aisle_aggregates.csv in precomputed mode already has aisle_id
        merged = agg
...
def build_aisle_network_data(min_count: int, min_lift: float, dim: int = 2):
    ...
    figure stays cheap.
    Works in both raw and precomputed modes — load_aisle_pairs() handles the
    data source switch transparently.
    """
    pairs, count_a, n_orders, aid2name = load_aisle_pairs()
...
# ── Neue Funktionen für den Precomputed-Modus ────────────────────────────────

@st.cache_data
def load_kpi():
    """Lade voraggerierte KPI-Skalare und Dept-Anteile (nur Precomputed-Modus).

    Returns
    -------
    (kpi, dept_share)
        kpi        : dict mit Schlüsseln total_orders, unique_products,
                     unique_customers, overall_reorder, avg_orders_per_customer,
                     avg_basket_size, top_department, top_dept_share,
                     peak_dow, peak_hour
        dept_share : pd.Series, Index=department, Values=Anteil 0–1
    """
    kpi = json.loads((PRECOMPUTED_DIR / "kpi.json").read_text())
    dept_share_df = pd.read_csv(PRECOMPUTED_DIR / "dept_share.csv")
    dept_share = dept_share_df.set_index("department")["share"]
    return kpi, dept_share


@st.cache_data
def load_shopping_heatmap(customer_type: str) -> pd.DataFrame:
    """Lade voraggerierten Heatmap-Pivot (nur Precomputed-Modus).

    Parameters
    ----------
    customer_type : "All orders" | "First-time customers" | "Repeat customers"

    Returns
    -------
    pd.DataFrame, 7 × 24, Index=Tagesname, Columns=0..23 (Stunden)
    """
    _DAY_NAMES = [
    ...
    df = pd.read_csv(
        PRECOMPUTED_DIR / f"shopping_heatmap_{key}.csv",
        index_col=0,
    )
