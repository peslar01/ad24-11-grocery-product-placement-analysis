...
| Product co-purchase network data | Computed from `order_products__prior.csv` | `data/data.pkl`, `data/ranking.pkl`, `data/pairs.pkl` |
...
7. Store the results as pickle files so the dashboard can load them with `@st.cache_data`.

The notebook `eda/graph_plot.ipynb` together with `eda/defs_graph_plot.py` runs these steps and writes the three output files.
...
with open("data/ranking.pkl", "rb") as f:
...
#### Pickle Data Catalogue

| File | Generated by | Loaded by | Content |
|---|---|---|---|
| `data/data.pkl` | `eda/graph_plot.ipynb`, `eda/defs_graph_plot.py` | `deployment/app.py` | Graph object with nodes (products) and edges (co-purchase relations) used to render the network. |
| `data/ranking.pkl` | `eda/graph_plot.ipynb`, `eda/defs_graph_plot.py` | `deployment/app.py` | Product ranking by purchase frequency, used for the top N slider and graph layout. |
| `data/pairs.pkl` | `eda/graph_plot.ipynb`, `eda/defs_graph_plot.py` | `deployment/app.py` | Pair table with co-occurrence counts and lift values for every product pair above the threshold. |
